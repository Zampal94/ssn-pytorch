...
from lib.dataset import tool2, augmentation
...
def train(cfg):
    ...
    checkpoint = torch.load("/home/janischl/ssn-pytorch/log/bset_model_tool.pth")
    ...
    while iterations < cfg.train_iter:
        for data in train_loader:
            ...
            #print(state)
            writer.add_scalar('train',metric, iterations)
            if (iterations % cfg.test_interval) == 0:
                ...
                if asa > max_val_asa:
                    ...
                    'optimizer_state_dict': optimizer.state_dict()}, os.path.join(cfg.out_dir, "bset_model_tool.pth"))
                    #torch.save(model.state_dict(), os.path.join(cfg.out_dir, "bset_model_tool.pth"))
